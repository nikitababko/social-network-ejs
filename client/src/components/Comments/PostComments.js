...
*/
    ...
    fetchData = () => {
        ...
        if (!comments[post.postId].fetching) {
            const lastId =
                comments[post.postId].comments[comments[post.postId].comments.length - 1]
                    ._id;
            dispatch(
    ...
    render() {
        ...
        if (comments[post.postId]) {
            ...
            const hasMore = fetchedComments.length === post.commentsCount ? false : true;
                        ...
                        {fetchedComments.length && hasMore ? (
                            <Fragment>
                                ...
                                <Button loading={fetching} onClick={this.fetchData}>
                                    Load {post.commentsCount - fetchedComments.length}{" "}
                                    more
                                </Button>
                            </Fragment>
                        ) : null}
